...
import (
	"minevillages/community/api/health"
	"minevillages/community/api/page"
	"minevillages/community/cache"
	"minevillages/community/json"
	"minevillages/community/util"
	"net/http"
	"path/filepath"
)
...
func (HTTPHandler) BackHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/api/health" {
		data = health.Health()
	} else if r.URL.Path == "/api/page" && r.Method == "POST" {
		pageCreateRequest := &page.PageCreateRequest{
			Title:      r.FormValue("title"),
			Permission: r.Header.Get("Authorization"),
		}
		data = pageCreateRequest.PageCreate()
	}
